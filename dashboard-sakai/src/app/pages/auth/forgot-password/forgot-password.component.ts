...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, ReactiveFormsModule, ButtonModule, InputTextModule, CardModule, RouterModule, AppFloatingConfigurator],
    template: ` 
    <app-floating-configurator></app-floating-configurator>
        <div style="padding: 0.3rem; background: linear-gradient(180deg, color-mix(in srgb, var(--primary-color), transparent 60%) 10%, var(--surface-ground) 30%)">

            <div class="mx-auto max-w-3xl p-6 bg-white rounded-lg shadow-lg dark:bg-gray-800 mt-60 w-full md:w-1/2 lg:w-1/3 h-full">
                <div class="text-center mb-4 flex flex-col items-center justify-center">
                    ...
                    <h3 class="text-900 text-5xl font-semibold mb-5">¿Olvidaste tu contraseña?</h3>
                ...
                <form [formGroup]="forgotPasswordForm" (ngSubmit)="onSubmit()" novalidate>
                    <div class="mb-8">
                        ...
                        <input id="email" type="email" formControlName="email" pInputText class="w-full text-sm" placeholder="correo@ejemplo.com" />
                        <small *ngIf="email.invalid && email.touched" class="text-red-500 text-xs">Correo válido requerido</small>
                    </div>

                    <button type="submit" pButton label="Enviar" class="w-full text-sm mt-2" [disabled]="forgotPasswordForm.invalid || loading"></button>
                    ...
                    <div class="flex justify-content-center mt-8">
                        <p-button routerLink="/" p-button label="Go to Dashboard" class="p-button-outlined p-button-sm"> </p-button>
                    </div>
                </form>
            </div>
        </div>
